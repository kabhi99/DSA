...
Say `arr = [1, 2, 3, 4, 5]`, pointers start at `left=0, right=4`:

`while (left < right)` — pointers stop BEFORE meeting:
```
Step 1: left=0, right=4  ✓ process
Step 2: left=1, right=3  ✓ process
Step 3: left=2, right=2  ✗ STOP (middle element NEVER processed)
```

`while (left <= right)` — pointers process the meeting point too:
```
Step 1: left=0, right=4  ✓ process
Step 2: left=1, right=3  ✓ process
Step 3: left=2, right=2  ✓ process  ← middle element processed!
...
```
+----------------------------------------+-------------------+-----------------------------+
| Pattern / Problem                      | Condition         | Why                         |
+----------------------------------------+-------------------+-----------------------------+
| Binary Search                          | left <= right     | Target may be at left==right|
| Two Sum (sorted) / Pair on ends        | left < right      | Can't pair element w/ itself|
| Container With Most Water              | left < right      | Need two distinct lines     |
| Valid Palindrome                       | left < right      | No need to compare middle   |
| Reverse String / Swap in place         | left < right      | Nothing to swap when meet   |
| 3Sum / 4Sum (inner two-pointer)        | left < right      | Need distinct elements      |
| Trapping Rain Water                    | left < right      | Pair of boundaries needed   |
| Squares of Sorted Array (fill backward)| left <= right     | Middle element still placed |
| Boats to Save People (greedy pair)     | left <= right     | Lone person still needs boat|
| Bag of Tokens (greedy)                 | left <= right     | Last token may still play   |
| Sort Colors (Dutch Flag: mid <= right) | mid  <= right     | Last unknown must be sorted |
+----------------------------------------+-------------------+-----------------------------+
```

**COMMON MISTAKES:**
- Using `left <= right` in Two Sum → ends up pairing `arr[i] + arr[i]` (invalid)
- Using `left < right` in Binary Search → misses target when it's the last element
